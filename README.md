# Hi, I'm Suman Kumar 👋

### Python Developer | AI/ML | Computer Vision | Backend Development

I'm a B.Tech Electronics & Communication Engineering student focused on building practical applications using Python, Artificial Intelligence, Machine Learning, Computer Vision, and Backend Development.

I enjoy solving real-world problems through software and continuously improving my skills by building and experimenting with projects.

---

## 🚀 About Me

- 🎓 B.Tech E&C Student — 5th Semester
- 🐍 Focused on Python Development
- 🤖 Learning Artificial Intelligence & Machine Learning
- 👁️ Interested in Computer Vision
- ⚡ Building backend applications with FastAPI
- 🔐 Currently developing an AI-driven cybersecurity project
- 🧠 Exploring Generative AI, LLMs and RAG
- 💼 Looking for AI/ML, Python, Backend or Computer Vision internship opportunities

---

## 🛠️ Tech Stack

### Programming
- Python
- SQL

### AI / Machine Learning
- NumPy
- Pandas
- Scikit-learn
- Machine Learning
- Data Preprocessing
- Exploratory Data Analysis
- Model Evaluation

### Computer Vision
- OpenCV
- YOLOv8

### Backend Development
- FastAPI
- REST APIs
- Pydantic
- Uvicorn
- SQLAlchemy

### Database
- SQLite

### Tools
- Git
- GitHub
- VS Code
- Jupyter Notebook

---

## ⭐ Featured Projects

### 🔐 QuantumShield

**AI-Driven Post-Quantum Secure Communication & Threat Detection Platform**

A Python/FastAPI-based cybersecurity platform designed to combine secure communication with AI-driven threat detection.

**Tech Stack:** Python · FastAPI · SQLAlchemy · SQLite · REST API · AI/ML



---

### 🚦 Dynamic Traffic Signal Control System

A computer-vision-based traffic management system using YOLOv8 to detect and analyze vehicles and support dynamic traffic signal decisions.

**Tech Stack:** Python · YOLOv8 · OpenCV · Computer Vision · Tkinter



---

## 📚 Currently Learning

- 🤖 Advanced Machine Learning
- 🧠 Generative AI
- 🔗 Large Language Models (LLMs)
- 📚 Retrieval-Augmented Generation (RAG)
- ⚙️ MLOps
- 🔐 AI for Cybersecurity

---

## 🎯 Career Interests

I'm interested in opportunities involving:

**Artificial Intelligence · Machine Learning · Computer Vision · Python Development · Backend Engineering · Generative AI **

---

## 📫 Connect With Me

- 💼 LinkedIn:https://www.linkedin.com/in/suman-kumar-2b180b35a/
- 📧 Email: sumankr62800@gmail.com
- 💻 GitHub: [@sumankr62800-del](https://github.com/sumankr62800-del)

---

⭐ Feel free to explore my repositories and follow my journey as I build projects in AI/ML and software development.

